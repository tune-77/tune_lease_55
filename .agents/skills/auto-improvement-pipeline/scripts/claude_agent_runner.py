...
def run_claude_agent(improvement: dict[str, Any], size: str) -> dict[str, Any]:
    ...
    pr_title = f"[needs-approval] feat: {title}"
    ...
    # NOTE: 以前はここで `gh pr merge --auto` を呼び、size=="auto" 判定の
    # 改善案を人間のレビューなしに master へ自動マージしていた。
    # リース審査という事業性質上、コード変更は必ず人間の承認を経るべきと判断し、
    # 自動マージは廃止した（2026-07-17）。PR作成のみ行い、マージは常に手動。
    merged = False
