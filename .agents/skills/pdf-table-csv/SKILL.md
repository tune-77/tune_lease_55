...
理由: OCR元は千円単位だが、バッチ審査テンプレートは百万円単位を期待するため、対象列を誤るとスコアが大きく壊れる。
適用条件: PDF/OCR由来CSVをバッチ審査テンプレートへ流し込む時。
削除条件: 変換スクリプトが列スキーマを検証し、単位変換対象外列を自動テストで保護できる時。
...
`部署`列は、OCR結果が次の営業部として読めた場合だけ正規化する。
...
読めない値やOCRノイズは空欄へ戻す。テンプレート流し込み時に空欄は`0`になる。特定部署へ無条件に寄せない。
...
理由: OCRは列ずれ、桁誤読、格付誤読が残るため、CSV生成成功だけでは審査投入可能とは言えない。
適用条件: OCRからCSVまたはバッチ審査テンプレートを作った直後。
削除条件: OCR結果の列数、主要列、格付、金額単位、テンプレート55列を自動検査する品質ゲートが標準化された時。
...
理由: 原本PDFの種類と向きによって最適な抽出経路が変わり、上書き事故も起きうる。
適用条件: PDFからCSVを抽出・整形・上書きする前後。
削除条件: スクリプト側でバックアップ、テキストPDF判定、向き補正が必ず実行される時。
