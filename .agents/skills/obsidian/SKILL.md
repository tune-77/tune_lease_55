...
理由: Obsidianは個人Vaultであり、外部公開・秘密情報・既存ノート破壊の事故が起きやすい。
適用条件: Vault探索、ノート作成、既存ノート編集、作業ログ保存を行う時。
削除条件: 保存先選択、秘密情報除外、追記方式がツール側で強制され、手順文なしでも検証できる時。
...
理由: 保存先・要約粒度・報告形式を固定しないと、通常Vaultとlease-wiki-vaultの混同や会話全文保存が起きる。
適用条件: ユーザーがObsidian/Vault保存を依頼した時。
削除条件: `obsidian_note.py` が保存先解決、要約整形、保存先報告まで一括で担保する時。
...
運用:
理由: 作業ログは再利用できる決定と検証だけを残し、会話全文や秘密情報を増やさないため。
適用条件: Codex/Claude作業ログ、日次メモ、プロジェクトメモを保存する時。
削除条件: ログ保存ツール側で全文除外と機密情報要約が強制される時。
