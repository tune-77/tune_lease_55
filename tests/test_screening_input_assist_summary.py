...
def test_screening_input_assist_summary_counts_copy_and_submit_metrics():
    rows = [
        ...
        },
        {
            "action": "copied",
            "session_id": "s1",
            "ts": "2026-08-15T09:01:00+00:00",
            "copied_field_count": 2,
            "confirm_count": 1,
        },
        {
            "action": "score_submitted",
            "session_id": "s1",
            "changed_after_copy_count": 10,
            "elapsed_ms": 90_000,
            "metadata": {"elapsed_from_copy_ms": "12000"},
        },
    ...
    assert summary["avg_copied_fields"] == 4.0
    assert summary["avg_confirm_fields"] == 2.0
    assert summary["avg_changed_after_copy"] == 10.0
    assert summary["avg_elapsed_after_copy_ms"] == 12_000
