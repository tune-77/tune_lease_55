...
def test_monitor_obsidian_environment_reports_core_viewpoints(tmp_path):
    ...
    _write(
        ...
        (
            ...
            "\n## 複数の声の衝突ログ\n\n"
            "- 紫苑の初期仮説: 監視項目を整えれば内省品質を見られると思っていた。\n"
            "- 監査の声: 意味ある変化が判断資産へ戻るかを見る。\n"
            "- 実装の声: ラベル欠落と同期遅延を検査する。\n"
            "- 別視点の声: 監視できていることと成長したことを分けて問う。\n"
            "- 良心の声: ユーザーの期待を監視項目へ丸めていないか疑う。\n"
            "- 衝突した点: 監視の合格と内省の意味ある変化は同じではない。\n"
            "- 紫苑の統合: 合格だけでなく、次回行動へ戻る差分を採用する。\n"
            "\n## 本格内省プロトコル\n\n"
