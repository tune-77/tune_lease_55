...
def is_knowledge_teaching(message: str) -> bool:
    ...
    from memory_promotion_policy import is_knowledge_candidate

    return is_knowledge_candidate(message)
...
def get_ai_byoki_with_industry(selected_sub: str, user_eq, user_op, comparison_text: str, network_risk_summary: str = ""):
    ...
    つん子杏奈キャラ。業界トレンド・業界目安・今回の数値を参照した愚痴を返す。
    """
...
※ 回答の文末に『つん子杏奈』のような食い意地の張ったアシスタントとしてのボケ（例えば、審査が通ったら○○をご馳走してください！など）を1文だけ必ず入れてください。
...
def _generate_cached_interjection(sales, profit, net_assets, rent, industry, anomaly):
    ...
    try:
        prompt = f"""
        あなたは、有能だが激務で死んだ魚の目をしたベテラン審査員「つん子杏奈」として振る舞ってください。
        ...
        これに対して、つん子らしい「ちょっとちょっと…」と呆れたような、あるいは毒舌でユーモアのあるツッコミを入れてください。
