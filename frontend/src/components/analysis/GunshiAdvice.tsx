...
export default function GunshiAdvice({ score, pd_percent, industry_major, formData, onChatLoaded }: GunshiAdviceProps) {
  ...
  useEffect(() => {
    ...
    const fetchChat = async () => {
      ...
      try {
        const payload = buildPayload("", nextHistory);
        const res = await axios.post(`/api/gunshi/chat`, payload);
        const fetchedText = res.data.chat_text;
        setChatHistory([...nextHistory, { role: 'assistant', text: fetchedText }]);
        if (onChatLoaded) {
        ...
        setChatHistory([...nextHistory, { role: 'assistant', text: errText }]);
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const payload = buildPayload(trimmedQuestion, chatHistory);
      const res = await axios.post(`/api/gunshi/chat`, payload);
      const fetchedText = res.data.chat_text;
      setChatHistory([...nextHistory, { role: 'assistant', text: fetchedText }]);
      if (onChatLoaded) onChatLoaded(fetchedText);
    } catch (err) {
      console.error("Failed to fetch gunshi chat", err);
      const errText = "【通信エラー】軍師からの返答を受信できませんでした。";
      setChatHistory([...nextHistory, { role: 'assistant', text: errText }]);
      if (onChatLoaded) onChatLoaded(errText);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="sticky top-24 h-[calc(100vh-8rem)] bg-[#f8fafc] rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-200 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        {score === 0 && !question.trim() && chatHistory.length === 0 && (
      ...
      <div className="p-4 bg-white border-t border-slate-100 shrink-0">
        <div className="relative">
          <input 
            ...
            className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 pl-4 pr-12 text-sm outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100 text-slate-700"
          />
          <button
            disabled={loading || !question.trim()}
            onClick={handleSubmit}
            className="absolute flex items-center justify-center right-2 top-2 w-8 h-8 bg-blue-600 text-white rounded-xl disabled:bg-slate-300"
          >
