...
export default function InterestPage() {
  ...
  const fetchAll = async () => {
    ...
    try {
      const [ratesRes, currentRes] = await Promise.all([
        axios.get("http://localhost:8000/api/settings/interest"),
        axios.get("http://localhost:8000/api/settings/interest/current"),
      ]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post("http://localhost:8000/api/settings/interest", payload);
  ...
  const handleSeed = async (overwrite: boolean) => {
    ...
    setSeedResult(null);
    try {
      const res = await axios.post(`http://localhost:8000/api/settings/interest/seed?overwrite=${overwrite}`);
