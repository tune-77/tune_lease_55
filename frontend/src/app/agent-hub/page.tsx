...
export default function AgentHubPage() {
  ...
  const fetchThoughts = async () => {
    try {
      const res = await axios.get("http://localhost:8000/api/agent_hub/thoughts?limit=30");
  ...
  const fetchLatestNovel = async () => {
    try {
      const res = await axios.get("http://localhost:8000/api/agent_hub/novel/latest");
  ...
  const runAgent = async (agentId: string) => {
    ...
    try {
      if (agentId === "novel") {
        const res = await axios.post("http://localhost:8000/api/agent_hub/novel/generate");
        setResult(res.data);
        fetchLatestNovel();
        fetchThoughts();
      } else {
