...
- **P0-1 パイプラインヘルスの注入**: `_build_dialogue_improvement_report_context()` に `data/pipeline_step_log.jsonl` 直近実行の失敗ステップ要約を追加。紫苑が「システム監視」報告を実データで行えるようにする
- **P0-2 台帳・キュー状況の注入**: `scripts/improvement_ledger.jsonl` 直近の applied/rejected と、Codex キュー（`reports/codex_auto_queue_*.json`）の実行結果サマリを追加
- **P0-3 再帰的自己改善レポート接続**: `reports/recursive_self_improvement_latest.md` の要点を相談材料へ追加

変更対象: `api/main.py`（context builder のみ）。DB/スコアリング/デプロイ設定に触れない。

完了条件: 対話室で「昨夜のパイプラインで失敗したステップは？」「今週マージされたREVは？」に紫苑が正答できる。

### Phase 1: トリアージの構造化記録（紫苑の判断を資産にする）

- **P1-1 トリアージ保存API**: `POST /api/improvement/triage`。候補の `canonical_key` に対し `今日やる / 後回し / 捨てる` と理由1行を `data/shion_improvement_triage.jsonl`（追記形式、最後のエントリ有効 — 台帳と同じ規約）へ記録
- **P1-2 対話室UI**: 改善PMレポートの各候補にトリアージボタンを追加。紫苑の提案分類を初期値にし、User が確定する
- **P1-3 翌日レポートへの反映**: 改善レポート文脈に「昨日のトリアージ結果と未処理分」を含め、判断の持ち越しをなくす

完了条件: トリアージ履歴が翌日の対話に「紫苑の判断＋User確定」として表示される。

### Phase 2: トリアージ→パイプライン接続（「実」を移す）

- **P2-1 キュー優先度の紫苑化**: `build_codex_auto_queue.py` が triage jsonl を読み、「今日やる（User確定済み）」を優先、「捨てる」を除外する
- **P2-2 自動承認の抑制入力**: `auto_approve_safe_recipes.py` に「紫苑が捨てると判断した候補は自動承認しない」条件を追加
- **P2-3 承認記録**: User が対話室で承認した時刻・対象を triage レコードへ記録（`approved_at`）。Codex 依頼文生成はこの承認レコードがある候補に限定

完了条件: 夜間パイプラインの実行順序・除外が「紫苑の分類＋User承認」で決まり、素通りルートが消える。
...
| KPI | 定義 | 目的 |
|-----|------|------|
| トリアージ網羅率 | 改善候補のうち紫苑が分類した割合 | 「中心」の実態確認 |
| リードタイム | 候補発生→トリアージ→PRマージまでの日数 | ループ速度 |
| 的中率 | 「今日やる」→マージ→品質スコア良好の割合 | 紫苑の判断品質 |
...
| 順 | 項目 | 変更対象 | リスク | 備考 |
|----|------|---------|--------|------|
| 1 | P0-1〜P0-3 | `api/main.py` context builder | 低（read-only） | 1 PR にまとめ可 |
| 2 | P1-1〜P1-3 | `api/main.py` + `api/schemas.py` + 対話室 page.tsx | 低〜中 | `/api/chat` の intent 分岐に触れないこと（CLAUDE.md 要注意領域） |
| 3 | P2-1〜P2-3 | `scripts/build_codex_auto_queue.py` / `auto_approve_safe_recipes.py` | 中 | パイプラインは追記のみ・`\|\| true` 維持（朝報告停止防止） |
| 4 | P3-1〜P3-3 | `scripts/cleanup_improvement_reviews.py` ほか | 中 | 台帳キーは `canonical_key(title)` 形式厳守 |
| 5 | Phase 4 | — | 高 | 前提ガード実装まで凍結 |
