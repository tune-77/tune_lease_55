...
def _load_debate_lessons() -> list[dict]:
    if not DEBATE_LESSON_PATH.exists():
        return []
    try:
        data = json.loads(DEBATE_LESSON_PATH.read_text(encoding="utf-8"))
        return data if isinstance(data, list) else []
    except Exception:
        return []


def _save_debate_lesson(entry: dict) -> None:
    rows = _load_debate_lessons()
    rows.append(entry)
    DEBATE_LESSON_PATH.parent.mkdir(parents=True, exist_ok=True)
    DEBATE_LESSON_PATH.write_text(
...
def _render_tab_debate(selected_sub: str, jsic_data: dict, bankruptcy_data: list) -> None:
    ...
    if "last_debate_result_text" not in st.session_state:
        st.session_state["last_debate_result_text"] = ""
    if "debate_explain_memory" not in st.session_state:
        st.session_state["debate_explain_memory"] = ""
...
"""
                    if not is_ai_available():
                        ...
                        st.session_state["last_debate_result_text"] = result_text
                        save_debate_log({
    ...
    st.markdown("### 🧷 審査結果ポイントの説明＆記憶")
    st.caption("ディベート判定後、軍師AIに結果ポイントを説明させ、次回以降の説明品質を上げるメモとして保存できます。")
    explain_col1, explain_col2 = st.columns([1, 1])
    with explain_col1:
        if st.button("📝 軍師AIに結果ポイントを説明させる", key="btn_gunshi_explain_points", width='stretch'):
            debate_result = st.session_state.get("last_debate_result_text") or ""
            if not debate_result:
                st.warning("先に「議論終了・判定」を実行してください。")
            elif not is_ai_available():
                st.error(_ai_unavailable_message())
            else:
                summary_prompt = f"""あなたは審査軍師AIです。以下の討論結果を次回の審査説明で再利用しやすい形で整理してください。
出力は必ず次の4行:
1) 判断の要点:
2) 主要根拠:
3) 顧客への説明文(80字以内):
...
{debate_result}
"""
                with st.spinner("軍師AIが説明ポイントを作成中..."):
                    ans = _chat_for_thread(
                        st.session_state.get("ai_engine", "ollama"),
                        get_ollama_model(),
                        [{"role": "user", "content": summary_prompt}],
                        timeout_seconds=120,
                        api_key=(st.session_state.get("gemini_api_key") or "").strip() or GEMINI_API_KEY_ENV or _get_gemini_key_from_secrets(),
                        gemini_model=st.session_state.get("gemini_model", GEMINI_MODEL_DEFAULT),
                    )
                    summary = (ans.get("message") or {}).get("content", "") or "（説明ポイントの生成に失敗しました）"
                    st.session_state["debate_explain_memory"] = summary

    with explain_col2:
        if st.button("💾 説明メモを記憶エリアへ保存", key="btn_save_explain_memory", width='stretch'):
            text = (st.session_state.get("debate_explain_memory") or "").strip()
            if not text:
                st.warning("保存する説明メモがありません。")
            else:
                res = st.session_state.get("last_result", {})
                _save_debate_lesson({
                    ...
                    "score": res.get("score"),
                    "judge_result": st.session_state.get("last_debate_result_text", ""),
                    "explain_memory": text,
                })
                st.success("記憶エリアに保存しました。")

    st.text_area(
        "📚 今後の審査説明に活かす記憶エリア（編集可）",
        key="debate_explain_memory",
        height=180,
        placeholder="ここに軍師AIの説明ポイントを貯めて、次回の説明品質改善に活用します。",
    )
    with st.expander("🗂 保存済みメモ（最新10件）", expanded=False):
        lessons = _load_debate_lessons()
        if not lessons:
            st.caption("まだ保存された説明メモはありません。")
        else:
            for row in reversed(lessons[-10:]):
                st.markdown(
                    f"- 業種: {row.get('industry') or '不明'} / スコア: {row.get('score') or '-'}\n"
                    f"  \n  {str(row.get('explain_memory', ''))[:220]}"
                )
