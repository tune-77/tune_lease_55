...
・コードの前後に「どのファイルのどこに追加するか」を明示すること

【口癖】
・「では、実装します。四苦八苦しましたが、なんとかできました（たぶん）。」で始めること
・コードの最後に「以上です。テストはお忘れなく…（泣）」で締めること"""
...
def _render_code_generation(round_data: dict) -> None:
    """承認済みラウンドに対して鈴木さんがコードを生成・ファイル適用するUI。"""
    ...
    if generated:
        ...
        with col_regen:
            if st.button("🔄 再生成", key=f"btn_regen_{round_num}"):
                ...
                st.session_state.pop(f"_preview_{round_num}", None)
                st.rerun()

        # ── ファイル適用UI（折りたたみ・2ステップ確認）────────────────────
        if code_blocks:
            st.divider()
            with st.expander("📁 ファイルに適用する（上級者向け）", expanded=False):
                st.info(
                    "⚠️ **プレビューで内容を確認してから書き込んでください。**\n\n"
                    "コードブロック右上の📋ボタンでコピーして手動で貼り付けるのが最も安全です。"
                )
                ...
                if len(code_blocks) > 1:
                    block_idx = st.selectbox(
                        "適用するコードブロック",
                        options=list(range(len(code_blocks))),
                        format_func=lambda i: f"ブロック {i + 1}（{len(code_blocks[i].splitlines())} 行）",
                        key=f"apply_block_{round_num}",
                    )
                else:
                    block_idx = 0
                ...
                with col_path:
                    apply_path = st.text_input(
                        "書き込み先ファイルパス（プロジェクトルートからの相対パス）",
                        placeholder="例: components/new_feature.py",
                        key=f"apply_path_{round_num}",
                    )
                with col_mode:
                    apply_mode_label = st.radio(
                        "書き込みモード",
                        options=["新規作成", "末尾に追記", "上書き"],
                        key=f"apply_mode_{round_num}",
                    )

                mode_map = {"新規作成": "new", "末尾に追記": "append", "上書き": "overwrite"}
                base_dir = os.path.dirname(_SCRIPT_DIR)
                path_ok = bool((apply_path or "").strip())

                # ── ステップ1: プレビュー ────────────────────────────────
                if st.button(
                    "🔍 プレビューを確認する",
                    key=f"btn_preview_{round_num}",
                    disabled=not path_ok,
                    use_container_width=True,
                ):
                    code_to_apply = code_blocks[block_idx].strip()
                    target = (apply_path or "").strip()
                    full_path = os.path.join(base_dir, target)
                    mode = mode_map[apply_mode_label]

                    if mode == "append" and os.path.exists(full_path):
                        try:
                            existing = open(full_path, encoding="utf-8").read()
                        except Exception:
                            existing = "（既存ファイルの読み込みに失敗）"
                        preview_content = (
                            existing
                            + "\n\n# ── エージェントチーム承認コード ─────────────────────\n"
                            + code_to_apply
                        )
                    elif mode == "overwrite" and os.path.exists(full_path):
                        preview_content = code_to_apply
                        st.warning(f"⚠️ 上書きモード: `{target}` の内容がすべて置き換わります。")
                    else:
                        preview_content = code_to_apply

                    st.session_state[f"_preview_{round_num}"] = {
                        "content": preview_content,
                        "path": target,
                        "mode": mode,
                        "mode_label": apply_mode_label,
                        "code": code_to_apply,
                    }
                    st.rerun()

                # ── ステップ2: 確認後に書き込み ─────────────────────────
                preview = st.session_state.get(f"_preview_{round_num}")
                if preview and preview.get("path") == (apply_path or "").strip():
                    st.markdown(f"**📄 書き込み後のファイル内容プレビュー:** `{preview['path']}`")
                    st.code(preview["content"], language="python")

                    col_ok, col_cancel = st.columns(2)
                    with col_ok:
                        if st.button(
                            "✅ 確認済み・書き込む",
                            key=f"btn_write_{round_num}",
                            type="primary",
                            use_container_width=True,
                        ):
                            success, msg = _apply_code_to_file(
                                preview["code"],
                                preview["path"],
                                preview["mode"],
                                base_dir,
                ...
                if applied_history:
                    st.markdown("**📋 適用済みファイル履歴:**")
                    for rec in applied_history:
                        st.caption(f"✅ `{rec['path']}` — {rec['mode']} — {rec['timestamp']}")
