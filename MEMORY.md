...
- **Cloud Run Memory Persistence**: DBそのものをGitへ保存する運用はしない。影響: 紫苑の動的記憶・内省・Cloud Run帰還データはGCS/DBへ保存し、Gitはコード・設計・静的知識だけに使う。次の行動: 今後この話題で毎回「DBをGit保存しますか」と聞かず、GCS event log → 隔離DB → 人間承認 → 本体DB昇格の方針を前提にする。
