...
if mode == "🏠 ホーム":
    ...
    import sqlite3 as _sq3, json as _json, time as _time
    from contextlib import closing as _cl
    _DB = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "screening_db.sqlite")
    _LDB = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "lease_data.db")
    ...
    def _get_pending():
        ...
        if os.path.exists(_DB):
            with _cl(_sq3.connect(_DB)) as c:
                ...
                rows = [dict(r) for r in c.execute("""
                    ...
                    WHERE (memo IS NULL OR memo='' OR memo NOT LIKE '%final_status%'
        ...
        if os.path.exists(_LDB):
            ...
            for r in rows:
                try:
                    m = _json.loads(r.get("memo") or "{}")
                    oc = m.get("_original_past_case_id")
                    if oc: seen.add(str(oc))
                except: pass
            with _cl(_sq3.connect(_LDB)) as c:
                c.row_factory = _sq3.Row
                for r in c.execute("SELECT id,timestamp,industry_sub,score,data FROM past_cases WHERE final_status='未登録' ORDER BY timestamp DESC").fetchall():
                    if str(r["id"]) in seen: continue
                    try: d = _json.loads(r["data"] or "{}")
                    except: d = {}
                    rows.append({"id": str(r["id"]), "created_at": r["timestamp"],
                        "industry_major": d.get("industry_major",""), "industry_sub": r["industry_sub"] or "",
                        "customer_type": d.get("customer_type",""), "score": r["score"] or 0,
                        "judgment": (d.get("result") or {}).get("hantei",""), "contract_prob": None,
                        "memo": r["data"], "_past": True})
        rows.sort(key=lambda x: x.get("created_at") or "", reverse=True)
        return rows

    _pending = _get_pending()
    st.caption(f"未登録件数: **{len(_pending)}件**")

    if not _pending:
        st.success("全ての案件が登録済みです！")
    else:
        _kw = st.text_input("🔍 絞り込み（会社名・業種・日付）", placeholder="例: 建設、2026-04")
        if _kw:
            _kw_l = _kw.lower()
            _pending = [r for r in _pending if
                _kw_l in (r.get("industry_sub") or "").lower() or
                _kw_l in (r.get("industry_major") or "").lower() or
                _kw_l in (r.get("created_at") or "").lower() or
                _kw_l in (_json.loads(r.get("memo") or "{}").get("company_name","") if r.get("memo","").startswith("{") else "").lower()
            ]
        st.caption(f"絞り込み後: {len(_pending)}件")
        _show = min(20, len(_pending))
        if len(_pending) > 5:
            _show = st.slider("表示件数", 5, min(100, len(_pending)), _show)

        for _i, _rec in enumerate(_pending[:_show]):
            _rid = _rec["id"]
            try: _m = _json.loads(_rec.get("memo") or "{}")
            except: _m = {}
            _company = _m.get("company_name","").strip()
            _ind = _rec.get("industry_sub") or _rec.get("industry_major") or "業種不明"
            _name = _company if _company else _ind
            _score = float(_rec.get("score") or 0)
            _judge = _rec.get("judgment") or "—"
            _date = str(_rec.get("created_at") or "")[:16]
            _is_past = _rec.get("_past", False)

            with st.expander(f"🏢 {_name}　｜　{_date}　｜　スコア: {_score:.1f}　｜　{_judge}", expanded=False):
                _c1, _c2 = st.columns([1, 2])
                with _c1:
                    st.write(f"**業種**: {_ind}")
                    st.write(f"**顧客区分**: {_rec.get('customer_type','—')}")
                    if _company: st.write(f"**会社名**: {_company}")
                    st.write(f"**判定**: {_judge}")
                    st.write(f"**スコア**: {_score:.1f}")
                    if st.button("🗑️ この案件を削除", key=f"_del_{_rid}_{_i}", type="secondary"):
                        if _is_past:
                            with _cl(_sq3.connect(_LDB)) as _c:
                                _c.execute("DELETE FROM past_cases WHERE id=?", (str(_rid),))
                                _c.commit()
                        else:
                            with _cl(_sq3.connect(_DB)) as _c:
                                _c.execute("DELETE FROM screening_records WHERE id=?", (_rid,))
                                _c.commit()
                        st.success(f"削除しました: {_name}")
                        _time.sleep(0.5)
                        st.rerun()
                with _c2:
                    with st.form(f"_reg_{_rid}_{_i}"):
                        _status = st.radio("結果", ["成約","失注"], horizontal=True)
                        _rate = st.number_input("獲得レート(%)", value=0.0, step=0.01, format="%.2f")
                        _lost = st.text_input("失注理由") if True else ""
                        _comp = st.text_input("競合他社")
                        _conds = st.multiselect("承認条件", ["本件限度","次回決算まで本件限度","金融機関と協調","独立・新設向け条件","親会社等保証","担保・保全あり","その他"])
                        _sub = st.form_submit_button("✅ 登録する", type="primary")
                    if _sub:
                        _extra = {"final_status": _status, "final_rate": _rate,
                                  "competitor_name": _comp.strip(), "loan_conditions": _conds}
                        if _status == "失注": _extra["lost_reason"] = _lost.strip()
                        if _is_past:
                            with _cl(_sq3.connect(_LDB)) as _c:
                                _row = _c.execute("SELECT data FROM past_cases WHERE id=?", (str(_rid),)).fetchone()
                                if _row:
                                    try: _d = _json.loads(_row[0])
                                    except: _d = {}
                                    _d.update(_extra)
                                    _c.execute("UPDATE past_cases SET final_status=?,data=? WHERE id=?",
                                        (_status, _json.dumps(_d, ensure_ascii=False), str(_rid)))
                                    _c.commit()
                        else:
                            with _cl(_sq3.connect(_DB)) as _c:
                                _row = _c.execute("SELECT memo FROM screening_records WHERE id=?", (_rid,)).fetchone()
                                if _row:
                                    try: _md = _json.loads(_row[0] or "{}")
                                    except: _md = {}
                                    _md.update(_extra)
                                    _c.execute("UPDATE screening_records SET memo=? WHERE id=?",
                                        (_json.dumps(_md, ensure_ascii=False), _rid))
                                    _c.commit()
                        st.success(f"✅ 登録完了: {_name} → **{_status}**")
                        _time.sleep(0.5)
                        st.rerun()
