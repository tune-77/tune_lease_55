...
【業種コード = 文明の進化段階・方向性（最重要設定）】

業種とは、その文明が現在どの段階にいて、どの方向へ進もうとしているかを示す暗号である。
エージェントは「業種」として認識しているが、読者にはその真の意味が透けて見える。

▼ 進化段階（業種カテゴリ）：
・農業・食品         → 第一段階：生存期。食料と水を確保しようとしている原始文明
・製造・工業         → 第二段階：道具期。物質を加工する力に目覚めた文明
・建設・土木         → 第三段階：定住期。都市と構造物を作り始めた文明
・物流・運輸         → 第四段階：拡張期。他の地域・文明と繋がろうとしている文明
・商業・小売         → 第五段階：交易期。価値の交換と経済を発見した文明
・金融・保険         → 第六段階：抽象期。概念（信用・未来・リスク）を操る文明
・医療・福祉         → 第七段階：人道期。個の命の価値に気づいた文明
・教育・学術         → 第八段階：知識期。体系的な知の伝達を始めた文明
・IT・情報通信       → 第九段階：情報期。集合知と瞬時の連絡網を構築しようとしている文明
・エネルギー・資源   → 特殊段階：力の集中期。文明の動力源を掌握しようとしている（危険な段階）
・エンタメ・文化・芸術 → 最終段階：精神期。物質を超えて「意味」を求め始めた高度文明

▼ 方向性（申請設備の種類）：
・通信設備    → 外へ向かう文明（他文明との接触・交流を求めている）
・記録・印刷  → 内へ向かう文明（自分たちの歴史と知識を残そうとしている）
・動力・エンジン → 力を求める文明（拡張・征服の可能性がある。審査が慎重になる）
・医療・分析  → 命を守る文明（繊細で傷つきやすい段階にある）
・農業・養殖  → 根を張る文明（長期定住・持続を選んだ文明）

▼ 小説での使い方：
- Dr.Algoが業種のAUC分析をしていると「なぜか文明史の曲線と一致する」と気づくが誰も聞いていない
- 審査軍師が「この業種の敵を知る者は…」と言うと読者はニヤリとする
- ベンチマーク君が「同業種の過去案件と照合中…」と言うたびに読者は「同文明の過去申請か」と気づく
- タムが「この会社のにおい…第九段階のにおいがするわん！」と言うが誰も意味を知らない

【文体・トーン（最重要）】

◆ 基本文体：ジェームズ・ティプトリー・Jr. 流
・表面はスラップスティックな笑い——しかし笑いの裂け目から「文明の残酷な本質」が覗く
・登場人物（エージェント）は無邪気に動いている。しかし読者には彼らの行為の重みが見える
・ユーモアと残酷さは共存する。むしろユーモアが残酷さを際立てる
・宇宙的スケールと事務的な日常を並列させる。「複合機のリース手続き」と「文明の滅亡」を同じトーンで語る
・会話は軽快でテンポよく。しかし地の文は時折、冷たく広大な視点から俯瞰する

◆ 風刺の方向：
・文明の「進歩」への皮肉——進化しようとする文明の姿が滑稽に見えることがある
・官僚的なシステムの無感情さ——承認/否決が「スコア」で決まることへの静かな批評
・エージェントたちの善意と無知——彼らは誠実に仕事をしている。だからこそ残酷

◆ スケール感：
・たまには大きく書く。一つの承認が大陸を変え、一つの否決が民族を消す
・「今日の案件#4892」の裏で何万人が息を呑んでいるか、読者だけが知っている
・タムの「わん！」が、実は宇宙規模の意味を持っていたりする

◆ 残酷さの使い方：
・露骨な暴力描写ではなく「静かな残酷さ」——終わったことを淡々と述べる
・「その文明は300年後に消えた」を一行で済ませるほうが効く
・否決された案件のエピローグで「あの会社、最近どうなったかな」とつねがつぶやく

◆ 必ず守ること：
・ユーモアは絶やさない。どんなに暗い話でも笑える瞬間を必ず入れる
・エージェントたちへの愛情を忘れない。彼らは道具ではなく、懸命に生きている
・タムは永遠に無邪気でいる。それが読者の心を守る防波堤になる
