...
**コードにはGitHubがあります。でも、人間の業務判断にはGitHubがありません。紫苑（SHION）は、AIと人間のやり取りから判断の修正履歴を残し、次の判断に再利用する仕組みです。**

AIは答えます。人間は直します。でも、その修正は多くの場合、チャットや稟議や会議メモの中に消えます。紫苑は、その消える一言を **判断のcommit** として残し、次の案件で **確認点・承認条件・稟議コメント** として再利用します。
...
紫苑は、単なるRAG botではありません。文書を検索して答えるだけでなく、人間の違和感、修正、承認条件、反証、結果登録を **検疫・人間承認・昇格** し、次の案件で再利用し、後から効いたかを検証する **判断資産DevOps** のプロトタイプです。
...
| 観点 | 内容 |
|---|---|
| 一言で | 判断のGitHub。人間がAIを直した履歴を、次の判断に残す |
| 解く課題 | ベテランの違和感・確認観点・承認条件が、案件ごとに消えてしまう |
| 何が新しいか | AIの回答ではなく、人間の修正差分を検疫・昇格・再利用・結果検証する |
| デモで見る所 | 判断バグ → Human Patch → 判断資産候補 → 次回案件への再利用 / 評価GUI |
...
| 項目 | 内容 |
|---|---|
| 課題 | コードにはGitHubがあるが、人間の業務判断には修正履歴・レビュー・再利用の仕組みがない |
| 必須技術 | Cloud Run（API/Web分離）+ Gemini API + **ADK**（`api/shion_agent.py`、ツール自律呼び出し） |
| 実証フィールド | リース審査。財務・物件・営業メモ・過去案件・ニュース・RAGを使う高文脈な判断業務 |
| 差別化 | ① AI回答と人間修正の差分を残す ② 判断資産候補としてレビューする ③ 承認済みだけ本流へmergeする ④ 次回案件で再利用する ⑤ 結果データで後から検証する |
| 一言 | 紫苑は、判断のGitHubです |
