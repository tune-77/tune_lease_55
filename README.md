...
多くの審査AIはスコアリングを目的にします。しかし実務の判断は、スコアの外側にある文脈、違和感、説明責任、条件設定で決まります。紫苑はその外側の判断を記録し、再利用可能な判断基準へ変換することで、AIエージェントが経験から判断を更新する仕組みを実装します。
...
| 差別化 | ① AI回答の観測→検知→内省→改善→検証→デプロイ ② 本番DBを守るデモ分離・検疫・昇格 ③ 記憶・反省・連続性を品質管理対象にする ④ Manaによる記憶汚染・攻撃文面の停止ゲート |
...
判断資産は単なるナレッジやメモではありません。案件レビュー、Auto Research、過去レビュー、人間の手入力から「次の審査で使える判断の型」を候補化し、実際の案件レビューで試します。
...
3. 人間が候補文面をその場で修正する
4. 人間が新しい判断を直接追加する
5. 修正済み・手入力判断を優先して次のレビューへ戻す
6. 紫苑が今回案件に合わせて判断を応用生成する
7. 使った判断資産は `判断資産出典: JA-...` として追跡する
8. `効いた / 微妙 / 外した` と結果検証で育てる

これは、AIに審査判断を教える **徒弟制度** に近い仕組みです。最初はAIが一般論を出し、人間が「それは当たり前」「この条件ならここを見る」「この文面に直す」と教えます。教えた判断は `edited_claim` や手入力候補として保存され、未編集候補より優先されます。
...
応用の強さは、審査画面で **保守的 / 標準 / 探索的 / 攻め** から調整できます。保守的では教えた判断に近い範囲で使い、探索的・攻めでは関連する新しい判断仮説まで広げます。ただし、探索的に出したものは正規の判断資産ではなく、人間の `効いた / 外した / 修正` を通して育てます。
