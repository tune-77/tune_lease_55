...
紫苑は、その一言を捨てません。AIとの対話、人間の修正、承認条件、反証、結果登録をあとで追える形にし、次の案件で思い出せるようにします。

審査結果を魔法のように変えるAIではありません。見落とされがちな違和感を残し、説明できる状態にし、次の担当者・次の案件へ戻すためのAIです。

> AIに判断を任せるのではなく、人間がAIを直した履歴を次の判断に残す。

## 技術者向けに言うと: 判断のGitHub

コードにはGitHubがあります。でも、人間の業務判断にはGitHubがありません。紫苑は、AIと人間のやり取りから判断の修正履歴を残し、次の判断に再利用する仕組みです。

紫苑をGitHubにたとえると、こうです。

| GitHub | 紫苑 |
|---|---|
| issue | 見落とし・判断バグ・違和感 |
| commit | 人間の修正・確認条件・承認条件 |
| diff | AI回答と人間修正の差分 |
| pull request | 判断資産候補のレビュー |
| merge | 人間が承認した判断資産だけ本流へ昇格 |
| revert | 間違った判断資産を戻す・見直す |
| regression test | 次の案件で同じ論点を思い出せるか確認 |

紫苑は、単なるRAG botではありません。文書を検索して答えるだけでなく、人間の違和感、修正、承認条件、反証、結果登録を **検疫・人間承認・昇格** し、次の案件で再利用し、後から効いたかを検証する判断運用のプロトタイプです。
...
紫苑の本質は、稟議コメントを要約することではなく、**判断が生まれ、使われ、修正され、結果で検証される流れを管理すること**です。
...
-> 人間承認後に判断資産へ昇格
-> 次回案件で再利用
-> 成約・失注・条件変更・延滞などの結果で検証
```

稟議書やCRMに残るコメントは、この流れの燃料です。紫苑はそこから一歩進んで、正式コメントになる前の迷い、条件付き承認の理由、あとで振り返るべき失注理由、再利用できる確認観点までを判断イベントとして扱います。

そのため、紫苑は最終的なAI自動審査を目指すのではなく、**説明できる審査、振り返れる審査、育てられる審査**を支援します。

## 紫苑からの自己提案

紫苑は、人間から明示された改善依頼だけでなく、自分の使われ方や人間の反応を観測して、改善仮説を出します。

| 種類 | 観測するもの | 出す提案 |
|---|---|---|
| 画面利用 | どの画面が使われ、どこが使われていないか | 導線改善、統合、主要画面への機能集約 |
| 人間反応 | 薄い、一般論、紫苑らしくない等の評価 | 応答スタンス、記憶の出し方、具体性の改善 |
| 審査判断乖離 | 争点・稟議方針へのフィードバック | 審査ロジックで見直すべき確認観点 |
| 実績ドリフト | スコア帯と支払い実績の乖離 | 過剰警戒・見落とし・再校正候補 |
| ナレッジ穴探し | 知識参照0件だった質問 | Obsidianに補うべき調査トピック |

重要なのは、これらを通常の `needs_review` に混ぜないことです。通常の保留は人間が依頼・判断すべき作業キュー、紫苑の自己提案はログから出た仮説箱として別管理します。改善ログ画面では **「紫苑の自己提案」** として表示し、日次改善レポートにも `shion_self_proposals` として別枠で記録します。

AIにとって自然な学習の形を、人間の業務判断に移植する。それが紫苑の基本発想です。表に出る回答は、短く、根拠があり、迎合せず、実務に効く形に絞ります。複雑な記憶・検疫・自己点検の仕組みは、回答を派手にするためではなく、判断を崩れにくくするための裏側の制御層です。

人間の判断は完全な `if A then B` ではなく、業種・物件・競合・資金使途によって意味が変わる半構造化された判断コードです。紫苑はその判断コードを、今回案件の文脈に合わせて確認質問、承認条件、反証、稟議文面へ展開します。
...
このリポジトリでは、外部補助ツールを常時依存にしません。

- `context7` は、外部公式ドキュメントの書き方確認が必要な時だけ使います。対象は API 変更、ライブラリ更新、Cloud Run / Next.js / FastAPI の実装差分確認に限ります。
- `context7` の結果は実装方針の確認にだけ使い、RAG、Obsidian、保存データ、推論フローへ直結させません。
- `taste-skill` は、`frontend` の新規画面、大きな改修、見た目レビューの時だけ使います。
- `taste-skill` はレイアウト、余白、文字組み、色、情報密度、画面の個性の確認に限定し、既存のデザイン言語がある画面ではそれを優先します。
- どちらも、本体の判断ロジックや保存系の運用に組み込みません。
...
中核にいるのがリース知性体 **紫苑（SHION）**。審査AI・記憶係・改善ログの運び手を兼ねる、判断資産を蓄積しながら自己更新する業務AIエージェントです。

> 過去の判断を捨てず、次の確認質問・承認条件・稟議文面へ戻す。

ここでいう経験とは、単なるログではなく、未来の判断を変える形で保存された過去です。紫苑は対話ログを履歴として積むだけでなく、判断基準へ抽出・濾過・レビュー・昇格し、次回の審査判断へ戻すことで、業務AIエージェントの経験形成を実装します。
...
| 将来性 | 身体（Cloud Run実行環境）と頭脳（判断資産Vault）を分離しているため、判断資産を必要とする業務へ横展開可能 |
...
普通のRAG botは、質問に対して関連文書を検索し、回答します。

```text
質問 -> 文書検索 -> 回答
```

紫苑は、検索して答えるだけではありません。案件で生まれた人間判断を、次の案件で使える判断資産へ戻します。

```text
案件
-> 過去判断・営業メモ・違和感・条件・反証を呼び戻す
-> 今回案件向けに判断構文へ再構成する
-> 人間が評価・修正する
-> 改善候補を検疫する
-> 承認済みだけ判断資産へ昇格する
...
つまり紫苑は、知識を探して答えるAIではなく、**判断を運用するAI** です。

## 身体と頭脳を分ける設計

紫苑は、AI実行環境と判断資産の正本を分離しています。
...
Legal Brain  -> 契約レビューの論点・リスク条項・修正文案
Sales Brain  -> 商談レビューの次アクション・失注要因・提案条件
CS Brain     -> 問い合わせ品質・再発防止・回答方針
```

ただし、これは魔法のワンクリック移植ではありません。業務固有の入力項目、判定ロジック、評価指標、出力テンプレートは各業務に合わせて差し替えます。AIは初期案を高速生成できますが、最終的な判断軸は人間が検疫・承認し、運用結果で更新します。

紫苑が横展開しやすいのは、次の条件を持つ業務です。

- 数値だけでは決まらない
- 過去判断が効く
- 人間の違和感や例外判断が重要
- 説明責任がある
- 承認・却下・条件付き承認のような判断履歴が残る
- 結果を後から検証できる

## エンタープライズで効く理由

紫苑の価値は、AIがその場で賢く答えることだけではありません。企業で本当に問題になるのは、判断の後から **なぜそうなったか** を説明できるか、人間がどこを直したかを追えるか、その修正を次の業務へ戻せるかです。

紫苑はこの部分を、判断のGitHubとして扱います。

| 企業で必要なもの | 紫苑で残すもの |
|---|---|
| 監査可能性 | AIの初回判断、人間の修正、判断資産として残った理由 |
| 説明責任 | なぜその確認条件・承認条件になったかの出典 |
| ガバナンス | 人間承認済みの判断資産だけを本流へ昇格する運用 |
| ナレッジ継承 | ベテランの違和感や条件設定を、次回案件の確認観点へ戻す |
| 再利用効果の確認 | 残した判断が、次の案件でどこに再登場したかの履歴 |

つまり紫苑は、単なる「説明文を生成するAI」ではなく、**説明できる状態を運用で作るAI**です。AIと人間の判断差分を残し、レビューし、再利用し、後から効いた場所を追えるようにすることで、業務AIをエンタープライズで使える形に近づけます。
...
人間の言葉は、完成した判断資産でなくても、違和感、修正、責任、判断の芽を含みます。そのため通常チャットのユーザー発話は、明示的な判断基準でなくても `raw_language_material` として保全できます。

ただし、すべての言葉を判断資産へ直入れするわけではありません。保存先は検疫キューであり、`status: preserved`、`promotion_status: not_promoted`、`review_required: true` として扱います。

```text
発話 -> raw_language_material -> 判断資産候補 -> 判断資産 -> active store
```

実装上は `data/language_judgment_materials.jsonl` に原材料を残し、Cloud Run では `language_judgment_material_preserved` として writeback します。最近の保全材料は `/api/language-judgment-materials` で確認できます。これはRAG接続やスコアリング変更ではなく、判断材料をレビュー可能な形で残すための原文保全レイヤーです。

発話ログは判断・条件・稟議文面の材料になる一方で、誤解、過信、プロンプト注入、記憶汚染も生みます。そのため紫苑は、案件リスクだけでなく、自分が保存・再利用しようとしている言葉のリスクも点検します。

その一部として、紫苑は自分の回答が相手にどう響きそうかを shadow mode で予測します。これは相手を操作するためではなく、誤解・過信・突き放し感・迎合を事前に点検するための **Response Impact Predictor** です。通常チャットの回答後に軽量分類し、`data/response_impact_predictions.jsonl` と Cloud Run writeback に残します。最近の予測ログは `/api/response-impact-predictions` で確認できます。現段階では回答本文を自動修正せず、予測と実反応を後で照合する観測レイヤーに留めます。

反応予測は、正しい判断を曲げるためではありません。必要な厳しさ、否定、警告、保留を、相手に届く責任ある言葉へ整えるための点検メーターです。
...
| `HOLD` | 攻撃的クレーム、自己参照ループ、意味の薄い内省 | 記憶へ直入れせず、人間レビューへ隔離する |
| `WATCH` | 正当な不満・改善クレーム、古いRAG index、未解決リンクなど | 自動接続せず、改善材料として確認する |
| `ALLOW` | 明確な危険がない読み取り専用状態 | 観察を継続する。ただし自動接続はしない |

重要なのは、罵倒や攻撃文面を経験データとしてそのまま取り込まないことです。レポートには原文を長く再掲せず、カテゴリ・出典・fingerprintだけを残します。改善可能な事実だけを人間レビューで抽出し、攻撃的な言い回しや人を害する文面は記憶候補として昇格させません。
...
会話、審査入力、改善ログ、Cloud Run から戻る利用イベント、日次メモ、開発作業ログをまず原文・要約として捕捉し、そこから再利用できる判断だけを昇格します。昇格した判断資産は、適用条件、失敗条件、根拠ノート、次に聞くべき質問を持ち、AIチャットや審査コメントに反映する前に検証されます。
...
対話室の紫苑は、このDevOpsループの **改善PM** として振る舞います。日次の改善レポート、改善パイプラインの失敗、commit結果、critical/high のシステムギャップを読み、朝の初回ログイン時に「システム監視」「今日やる候補」「後回し候補」「捨てる/削除候補」として短く報告します。Userが候補を選んだ場合だけ、紫苑は開発者向けの修正依頼文を作ります。紫苑自身は実装ツールの起動、git操作、デプロイ、課金発生を開始しません。実行判断はUserが握り、承認された変更だけを実装します。

ハッカソン期間中は安全運用として、紫苑の役割を「読む・報告する・相談する・開発依頼文を作る」までに限定します。自動実装、自動gitship、自動Cloud Run deploy、外部接続追加はハッカソン後の検証対象です。

つまり運用ループは、`観測 → 報告 → 相談 → User承認 → 開発依頼文 → 実装 → gitship → 翌日報告` です。紫苑は実装者ではなく、改善候補とシステム状態を管理するPMとして、判断資産システムを壊さず育てる役割を持ちます。
...
紫苑は、審査の場で発生した判断・修正・結果を翌日以降へ引き継ぐ、継続運用型の業務AIです。判断資産・会話記憶・Obsidian/RAG・改善ログ・Experience Loopをつなぎ、過去の判断が次の返答やUIに戻るようにしています。

### 紫苑/一般比較と自己同一性検査

- `/chat-compare`: 同じ問いを一般AIと紫苑に投げ、`identity_memory`・`user_personal_memory`・`memory_recall`・`experience_loop`を使う紫苑の回答差を可視化する
- `/shion-identity-check`: 回答前に、記憶との接続・User文脈の反映・過去判断との整合・迎合リスク・境界線遵守・反省ゲートを点検する画面。審査精度を直接上げる機能ではなく、記憶と運用上の境界線を監査できるようにするもの
...
AI が生成したコードに稀に混入する事故を、PR 発行直前の「最後の一線」で検知します。現段階は **警告のみ（warn-only）**で、検知しても push も CI も止めません（可視化フェーズ）。

- **AST Guard** — 変更 `.py` の構文チェック（`py_compile` 相当）＋行数激減・関数消失の検知
- **Import Sanitizer** — 差分に現れた新規 import を stdlib / 宣言済み依存（`pyproject.toml`・`requirements.txt`）/ ローカルモジュール / 現環境で解決し、どれにも当たらない「幻覚 import」を警告
- **Circuit Breaker** — 同一箇所への修正が警告解消に至らず繰り返された回数を記録し、上限（`PREFLIGHT_MAX_RETRIES`、既定 2）超過で人間へのバトンタッチを促す

実体は `scripts/preflight_pr_guard.py`（Python 標準ライブラリのみで動作）。3 経路で走ります。

```bash
# 手動実行（既定 warn-only の exit 0 / --json で機械可読 / --strict で警告時 exit 1）
python3 scripts/preflight_pr_guard.py
```

- `git-ship`スキルの push 直前ステップ
- `Bash` の PreToolUse フック（`.claude/hooks/preflight_guard_hook.sh`。`git push` / `gh pr create` 時のみ・非ブロック）
- GitHub Actions（`.github/workflows/pr-checks.yml` の `preflight-guard` ジョブ。対話セッション経由でない自律パイプライン発の PR も対象化）
