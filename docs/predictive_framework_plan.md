...
1. **数値予測の実績採点はshadow運用を開始済み**。`POST /api/future-simulation/actuals` で実績日を必須として後日実績を `data/prediction_numeric_actuals.jsonl` へ追記し、東京時間の暦日で実績日以前に作成された予測だけを対象に、予測年ごとの売上・営業利益について MAE / MAPE / 80%区間包含率を日次レポートで測る。突合5件未満は参考値とし、スコア・判定・モデルへ自動反映しない
