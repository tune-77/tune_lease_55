...
記憶の種類だけでなく、時間の長さでも扱いを分ける。これを分けないと、直前の会話メモ、数日続く圧点、長期の判断基準が同じ棚に入り、想起がランダムに見えやすい。

| layer | window | purpose | surface rule |
|---|---|---|---|
| 短期記憶 | 数分〜1日 | 直前の訂正、会話の流れ、重複質問の回避 | 原則として明示しない。返答の自然さと質問精度にだけ出す |
| 中期記憶 | 2日〜数週間 | 話題の継続、不満の再発、前提変更、次回振る舞い候補 | 差分レポートやレビューでは見せる。通常会話では必要時だけ薄く使う |
| 長期記憶 | 数週間以上 | 判断基準、価値観、設計原則、ユーザーの持続的な好み | レビュー後に昇格する。短期の気分や単発作業ログをそのまま入れない |

差分を見る目的は、記憶量を増やすことではない。昨日から何が変わったか、何が繰り返されているか、何を長期記憶へ昇格すべきかを分けるためである。

実装メモ:

- `scripts/build_shion_timeline_delta.py` が `memory/YYYY-MM-DD.md` の直近差分から `short_term` / `mid_term` / `long_term` を出す。
- `short_term` は次回応答の自然さに使うが、「前回は...」という記憶アピールには使わない。
- `mid_term` は1日前/2日前/3日前などの継続語・新規語・消えた語を見る。
- `long_term` はレビュー待ちの昇格候補であり、自動でプロンプトや判断基準へ混ぜない。
