...
| `scripts/obsidian_theme_radar.py` | Vault全体の繰り返しテーマ上位3件を抽出し、Vertex AI Google検索グラウンディングで実際の反響（バズ）を確認。重なるテーマがあれば切り口を1つ提案 | なし | Vertex AI（Google検索グラウンディング＋テキスト生成） |
| `scripts/obsidian_reflection_journal.py` | `obsidian_theme_radar`の結果をProblem/気づき/解決の3段アウトラインにし、`Content Strategy Journal/`へ日次ノートとして記録 | あり | Vertex AI（テキスト生成） |
...
`obsidian_reflection_journal.py` が書くのはユーザー自身のコンテンツ企画メモであり、
性質が異なるため `Content Strategy Journal/`（Vaultルート直下の新規フォルダ）に分離した。
...
- 処理対象は `Private Reflection` / `Dialogue` / `AI Chat` / `Vertex Distilled` /
  `Content Strategy Journal` など運用・自己生成フォルダを除外し、自己参照ループを避ける
