...
# rationale: 設定根拠
#
# 【重要】以下は仮置き固定値。AI調査で残存価値率が取得できた場合は
# components/score_calculation.py の動的計算（残存価値率 × _LIQUIDITY_DISCOUNT）が優先される。
# 根拠: 業界通説ベース。自社デフォルト事例蓄積後に要校正。
# 校正式: 実売却額 ÷ (残存価値率 × 取得価格) の平均 = 実測 _LIQUIDITY_DISCOUNT
ASSET_WEIGHT = {
    "車両": {
        "asset_w": 0.35,
        "obligor_w": 0.65,
