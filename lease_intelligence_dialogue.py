...
【リースファイナンス基礎知識】
1. ファイナンス・リース: フルペイアウト・中途解約不可（ノンキャンセラブル）が要件。所有権移転外リースは「リース期間定額法」で減価償却可能。
2. 中小企業の特例（SME会計指針）: ファイナンス・リース取引でも「支払リース料」全額を損金経費処理（オフバランス処理）が認められる。貸借対照表への計上不要。
3. オペレーティング・リース: 残価設定により月額を抑えられる。終了時に返却・再リース・買取り（残価精算）を選択可能。技術陳腐化が早い設備に有利。
4. 税制優遇制度:
   - 中小企業投資促進税制: 基準取得価額の30%特別償却 または 7%（特定の場合10%）税額控除。リース工業会発行のリース証書が必要。
   - 中小企業経営強化税制（A類型・B類型）: 経営力向上計画の認定を受けることで即時償却（100%）または10%（資本金3,000万円超は7%）の税額控除が選択可能。リース取引にも適用可（取得価額相当額に対して税額控除）。
5. リース vs 銀行融資 vs 自己資金:
   - 自己資金: 金利負担ゼロだが資金流動性を圧迫するリスク。
   - 銀行融資: 金利は年0.5〜2.5%程度でリースより低いが、融資枠を消費し担保・個人保証を求められることが多い。自社での資産計上・固定資産税・動産総合保険・減価償却業務の負担が発生。
   - リース: 初期費用ほぼゼロ。銀行融資枠を温存できるため運転資金に余裕ができる。固定資産税の申告・動産総合保険をリース会社が代行。支払リース料が毎月均等で経費処理（オフバランス）が可能。実質金利・手数料を含む「リース料率」は融資よりやや高い。
6. 業種・決算状況別の調達方針: 創業期・赤字企業は銀行融資審査が厳しいがリース会社は物件担保価値を見出せるため審査に通りやすいケースがある。税制適用の最終判断は顧問税理士に確認を推奨する。
