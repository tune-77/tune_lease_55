...
Phase 7 の目的: リース業界固有の同義語・専門用語を集約したドメイン辞書を整備し（P7-001）、RAG検索のクエリ拡張に活用して検索の取りこぼしを減らす（P7-002）。`docs/archive/代替案実装計画_精度改善Ver2.md` 代替案Aの詳細設計。スコアリング（RF/LGBM・量子リスク）には一切触れない。
...
- 計画書（`docs/archive/代替案実装計画_精度改善Ver2.md`）は `mobile_app/` を対象としているが、現行の本番RAG経路は `api/knowledge/vector_store.py` のため、本Phaseは api 側を正とする。配置変更の理由は P7-001 の「計画書からの変更点」を参照。
